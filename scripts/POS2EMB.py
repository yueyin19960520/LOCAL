...
class POS2EMB():
    ...
    def get_targets_for_next_loop(self, pos2e_res, num_for_next_loop=2000): #Only choose the best one?
        ...
        for (pos2e_dataset, pos2e_model, setting, PRED_DICT, top_k_MAE, suffix) in pos2e_res: #only one
            ...
            temp = [i[0] for i in sorted_result]
            ...
            analogue_counts = []
            chosen_num = lambda mae: min(np.ceil(np.exp(50*mae**2))-2, 9)
            for bad_name, bad_ae, bad_emb in sorted_result:
                analogue_counts.append(chosen_num(bad_ae))

            # Scale analogue_counts to ensure total does not exceed 2000
            total_analogues = sum(analogue_counts)
            if total_analogues > num_for_next_loop:
                scale_factor = num_for_next_loop / total_analogues
                scaled_analogue_counts = []
                analogue_counts = [int(np.ceil(n * scale_factor)) for n in analogue_counts]
