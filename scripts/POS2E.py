...
class POS2E():
    def __init__(self, COHP_info_dict=None, splitted_keys=None, config=None, loop=0):
        ...
        self.prefix = "POS2E_edge_Net" if self.edge_involved else "POS2E_Net"
    ...
    def train_single_model(self, setting, dataset, PRED, finetune):
        ...
        if finetune:
            model.load_state_dict(torch.load(os.path.join(self.root_path, "models", "%s_%s_loop%s.pth"%(self.prefix, suffix, 0)))) #self.loop-1
            optimizer = torch.optim.AdamW(model.parameters(), lr=self.learning_rate*0.5, weight_decay=self.weight_decay)
        else:
        ...
        best = np.float64('inf')
        if finetune:
            log_file = os.path.join(self.root_path, "models", "%s_%s_loop%s.txt"%(self.prefix, suffix, self.loop))
            model_file = os.path.join(self.root_path, "models", "%s_%s_loop%s.pth"%(self.prefix, suffix, self.loop))
        else:
            log_file = os.path.join(self.root_path, "models", "%s_%s.txt"%(self.prefix, suffix))
            model_file = os.path.join(self.root_path, "models", "%s_%s.pth"%(self.prefix, suffix))
    ...
    def get_all_models(self, finetune=False):
        dataset_model_dict = {}
        for suffix, [dataset, model, setting, PRED] in self.COHP_info_dict.items():
            if not self.edge_involved:
            ...
            if finetune:
                ...
                model.load_state_dict(torch.load(os.path.join(self.root_path, "models", "%s_%s.pth"%(self.prefix,suffix)))) 
    ...
    def get_all_models_sorted(self, dataset_model_dict_with_PRED_DICT=None, top_k=0.1):
        ...
        for suffix, [dataset, model, setting, PRED_DICT] in dataset_model_dict_with_PRED_DICT.items():
            ...
            with open("./next_loop/sorted_result_%s.pkl"%datetime.datetime.now().strftime("%Y%m%d%H%M%S"), "wb") as f:
