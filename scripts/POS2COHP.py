...
class POS2COHP():
    ...
    def train_single_model(self, setting, finetune, mix_ratio):
        ...
        if finetune:
            model.load_state_dict(torch.load(os.path.join(self.root_path, "models", "_POS2COHP_Net_%s.pth"%suffix))) 
            optimizer = torch.optim.Adam(params=model.parameters(), lr=self.learning_rate*1e-1, weight_decay=self.weight_decay)
        else:
    ...
    def get_all_models(self):
        ...
        for setting in self.combinations_settings:
            ...
            key_list = [i for j in self.splitted_keys.values() for i in j]
            dataset = CombinedDataset([POS2COHP_Dataset(self.root_path, self.Element_List, setting, loop=l) for l in range(self.loop + 1)])
