...
def construct_run_list(root):
    ...
    for i in l1:
        ...
        if os.path.exists(kpoints): 
            content = os.popen('grep "aborting loop because EDIFF is reached" %s' % (outcar)).read()
...
def run(p_dir, folder, root):
    ...
    incar = os.path.join(p_dir, "INPUT", "INCAR.static")
    ...
    os.system("source /opt/intel/oneapi/setvars.sh && export PATH=/data/scicons/vasp6.3.0/vasp.6.3.0/bin:$PATH")
