...
class POS2COHP_Dataset(Dataset):
    ...
    def raw_file_names(self):
        if self.loop > 0:
            return ["icohp_structures_loop%s.pkl"%str(self.loop)]
        else:
    ...
    def processed_file_names(self):
        if self.loop > 0:
            return ["POS2COHP_%s_loop%s.pt"%(self.suffix,str(self.loop))]
        else:
...
class POS2E_edge_Dataset(Dataset):
    ...
    @property
    def raw_file_names(self):
        return ["raw_energy_data_dict_all.pkl"] + ["raw_energy_data_dict_loop%s.pkl"%str(l) for l in range(1, self.loop + 1)]
