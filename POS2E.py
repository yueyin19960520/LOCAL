...
class POS2E():
    ...
    def train_single_model(self, suffix, dataset, setting, PRED,Data_Augmentation,shuffle):
        print(setting)
        if shuffle:
            print('shuffle')
            pos2e_dataset = POS2E_Dataset("./", dataset, PRED, self.raw_data_dict, 
                                      setting, suffix,Data_Augmentation=Data_Augmentation).shuffle()
        else:
            pos2e_dataset = POS2E_Dataset("./", dataset, PRED, self.raw_data_dict, 
                                      setting, suffix,Data_Augmentation=Data_Augmentation)#.shuffle()
