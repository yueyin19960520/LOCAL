...
class POS2E():
    ...
    def train_single_model_with_active_learning(self, suffix, dataset, setting, PRED, verbose):
        ...
        for iteration in range(4): 
            ...
            tr_top_indices = np.argsort(tr_losses)[-int(len(pos2e_dataset) * 0.03):] # 训练集中表现不好的，总数的0.03，否则每轮加的数量不一样
            ...
            for i in tr_top_indices:
                dd,analogue_idx = pool_kd_tree.query(tr_embeddings_list[i],3,workers=2)
                if verbose:
