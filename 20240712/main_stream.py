...
for i in range(5):
    ...
    file_path = "processed/POS2COHP_FcN_RG_Homo.pt"
    # 检查文件是否存在，如果存在则删除
    if os.path.exists(file_path):
        os.remove(file_path)
        print(f"文件已删除：{file_path}")

    file_path = "processed/POS2EwithoutCOHP.pt"
    # 检查文件是否存在，如果存在则删除
    if os.path.exists(file_path):
        os.remove(file_path)
        print(f"文件已删除：{file_path}")

    file_path = "processed/CONT2EwithoutCOHP.pt"
    # 检查文件是否存在，如果存在则删除
    if os.path.exists(file_path):
        os.remove(file_path)
        print(f"文件已删除：{file_path}")

    file_path = "processed/POS2E_edge_FcN_RG_Homo.pt"
    # 检查文件是否存在，如果存在则删除
    if os.path.exists(file_path):
        os.remove(file_path)
        print(f"文件已删除：{file_path}")

    file_path = "processed/POS2COHP2E.pt"
    # 检查文件是否存在，如果存在则删除
    if os.path.exists(file_path):
        os.remove(file_path)
        print(f"文件已删除：{file_path}")

    with open('raw/icohp_structures_all.pkl', "rb") as pklf:
        ...
        # 构建文件名
        filename = f"{folder_name}/list_{timestamp}.pkl"
    # 将列表使用pickle保存到文件
        with open(filename, 'wb') as file:
            pickle.dump(icohp_list_keys, file)
    ...
    pos2cohp = POS2COHP(Element_List, setting_dict,
                        ...
                        predictor_hidden_feats=128, epochs=100, verbose=True,part_MN=True)
    pos2cohp.build_raw_data_dict()
    if model_training_flags["POS2COHP"]:
        pos2cohp_dataset = pos2cohp.train_all_models(enc_type='onehot',icohp_list_keys=icohp_list_keys) # encode type
    import pickle
    with open('./utilized_POS2COHP_dataset','wb') as file:
        pickle.dump(pos2cohp_dataset, file)
    dataset_model_dict_with_PRED = pos2cohp.build_bridge_for_E(pos2cohp_dataset)
