...
class POS2COHP2E:
    ...
    def build_raw_data_dict(self):   
        if not os.path.exists(os.path.join("raw", "icohp_structures_all.pkl")):
            ...
            print("COHP raw data dict prepare done!")
        else:
            print("COHP raw data dict already exist!")
        
        if not os.path.exists(os.path.join("raw", "raw_energy_data_dict_all.pkl")):
            self.raw_data_dict_COHP = build_raw_DSAC_file("./", "structures_all")
            print("E raw data dict prepare done!")
        else:
            print("E raw data dict already exist!")
            file_get = open(os.path.join("raw","raw_energy_data_dict_all.pkl"),'rb') 
            self.raw_data_dict_E = pickle.load(file_get) 
            file_get.close()
        return None
    ...
    def train_single_model(self, setting,icohp_list_keys):
        ...
        #####################################################################################################################
        pos2e_without_cohp_dataset = POS2COHP2E_Dataset("./", self.Element_List, setting, suffix,icohp_list_keys=icohp_list_keys,
                                                        part_MN=self.part_MN, raw_data_dict_E=self.raw_data_dict_E)#.shuffle() shuffle bas been done in main()
        ...
        print("POS2COHP2E whole dataset:%s, Trainig set:%s, Valid set:%s, Test set:%s."%(len(pos2e_without_cohp_dataset),
        ...
        model = POS2COHP2E_Net(in_feats=node_feats, hidden_feats=self.hidden_feats, activation=None, residual=None, batchnorm=None, 
                                     predictor_hidden_feats=self.predictor_hidden_feats, n_tasks=n_tasks, predictor_dropout=0.
                                     
                                     ,dim=self.dim, linear_dim_list=self.linear_dim_list,
                           conv_dim_list=self.conv_dim_list,
                            adj_conv=False, in_features=node_feats, bias=False,
                               conv=pyg_GCNLayer_without_edge_attr, dropout_E=0., dropout_COHP=None, pool=GraphMultisetTransformer,
        ...
        best = 1e5
        log_file = "./models/POS2COHP2E_Net_%s_%s.txt"%(suffix,file_name_current_time)
        ...
        for i in range(epochs):
            ...
            train_loss, train_res = cont2e_train_serial(model, pos2e_without_cohp_train_loader, combined_loss, optimizer, 0.01, 1)
            valid_loss, valid_res = cont2e_evaluate_serial(model, pos2e_without_cohp_valid_loader, loss_func)
            test_loss, test_res = cont2e_evaluate_serial(model, pos2e_without_cohp_test_loader, loss_func)
            ...
            if learning_rate <= 1e-5 and valid_res < best:
                torch.save(model, "./models/POS2COHP2E_Net_%s%s.pth"%(suffix,file_name_current_time))
                best = valid_res
    ...
    def train_all_models(self,icohp_list_keys):
        ...
        print("Finish the training of POS2COHP2E models.")
