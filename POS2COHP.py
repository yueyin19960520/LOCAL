...
class POS2COHP():
    ...
    def train_single_model(self, setting):
        ...
        ###########################################################################################
        # node_feats = len(self.Element_List)+15+1 if setting["Fake_Carbon"] else len(self.Element_List)+15
        node_feats = 22
        n_tasks = 2 if setting["Binary_COHP"] else 1

        pos2cohp_dataset = POS2COHP_Dataset("./", self.Element_List, setting, suffix).shuffle()
