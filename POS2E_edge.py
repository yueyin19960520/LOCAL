...
class POS2E_edge():
    def __init__(self, COHP_info_dict, split_ratio=0.8, batch_size=48, dim=256, epochs=300, verbose=True,
                 linear_dim_list = [], conv_dim_list = [], edge_dim = None):
    ...
    def train_single_model(self, suffix, dataset, setting, PRED,pool_type,
                           aug,maximum_num_atoms,real_cohp,noise,noise_type,noise_mae,icohp_list_keys,train_with_real=False):
        ...
        print(setting)
        pos2e_dataset = POS2E_edge_Dataset("./", dataset, PRED, self.raw_data_dict, setting, suffix,
                                      Data_Augmentation=aug, maximum_num_atoms=maximum_num_atoms,
                                      real_cohp=real_cohp,icohp_list_keys=icohp_list_keys)#.shuffle()
        #pos2e_dataset = POS2E_Dataset_Augmentation(pos2e_dataset, suffix)

        pos2e_train_dataset = pos2e_dataset[:int(len(pos2e_dataset)*self.split_ratio[0])].shuffle()
        pos2e_valid_dataset = pos2e_dataset[int(len(pos2e_dataset)*self.split_ratio[0]):int(len(pos2e_dataset)*(self.split_ratio[0]+self.split_ratio[1]))]
        pos2e_test_dataset = pos2e_dataset[int(len(pos2e_dataset)*(self.split_ratio[0]+self.split_ratio[1])):]

        
        pos2e_train_loader = DataLoader(pos2e_train_dataset, batch_size=self.batch_size, shuffle=True, drop_last=True)
        pos2e_valid_loader = DataLoader(pos2e_valid_dataset, batch_size=self.batch_size, shuffle=False, drop_last=False)
        pos2e_test_loader = DataLoader(pos2e_test_dataset, batch_size=self.batch_size, shuffle=False, drop_last=False) 

        print("Net-by-Net whole dataset:%s, Trainig set:%s, Valid set:%s, Test set:%s."%(len(pos2e_dataset), len(pos2e_train_dataset), len(pos2e_valid_dataset), len(pos2e_test_dataset)))
        ...
        model = CONT2E_Net(dim=self.dim, linear_dim_list=self.linear_dim_list,
                           conv_dim_list=self.conv_dim_list,
                            adj_conv=False, in_features=node_feats, bias=False,
                               conv=pyg_GCNLayer_with_edge, dropout=0., pool=GraphMultisetTransformer,
                               pool_ratio=0.25, pool_heads=4, pool_seq=["GMPool_G"], pool_layer_norm=False,
                               pool_type=pool_type,edge_dim=self.edge_dim,noise=noise,noise_type=noise_type,noise_mae=noise_mae,real_cohp=real_cohp)
        print(model)
        epochs = self.epochs
        ...
        best = 1e5
        if train_with_real:
            log_file = "./models/POS2E_edge_Net_train_with_real_%s_%s.txt"%(suffix,file_name_current_time)
        else:
            log_file = "./models/POS2E_edge_Net_%s_%s.txt"%(suffix,file_name_current_time)
        with open(log_file, 'w') as file:
            file.close()
        noise_info = f'train_with_real? {train_with_real} real cohp? {real_cohp},noise:{noise},noise_type:{noise_type},noise_mae:{noise_mae}'
        with open(log_file, 'a') as file:
            file.write(noise_info + "\n")
        for i in range(epochs):
            ...
            if not train_with_real:
                train_loss, train_res = cont2e_train(model, pos2e_train_loader, loss_func, optimizer)
                valid_loss, valid_res = cont2e_evaluate(model, pos2e_valid_loader, loss_func)
                test_loss, test_res = cont2e_evaluate(model, pos2e_test_loader, loss_func)
            if train_with_real:
                train_loss, train_res = cont2e_train(model, pos2e_train_loader, loss_func, optimizer,train_with_real=True,is_trainning=True)
                valid_loss, valid_res = cont2e_evaluate(model, pos2e_valid_loader, loss_func,train_with_real=True,is_trainning=False)
                test_loss, test_res = cont2e_evaluate(model, pos2e_test_loader, loss_func,train_with_real=True,is_trainning=False)
            ...
            if learning_rate <= 1e-5 and valid_res < best:
                if train_with_real:
                    torch.save(model, "./models/POS2E_Net_train_with_real_%s_%s.pth"%(suffix,file_name_current_time))
                else:
                    torch.save(model, "./models/POS2E_Net_%s_%s.pth"%(suffix,file_name_current_time))
                best = valid_res
