...
class POS2E_without_COHP:
    ...
    def train_single_model(self, setting):
        ...
        #####################################################################################################################
        pos2e_without_cohp_dataset = POS2EwithoutCOHP_Dataset("./", self.Element_List, setting, suffix).shuffle()

        pos2e_without_cohp_tr_dataset = pos2e_without_cohp_dataset[:int(self.split_ratio*len(pos2e_without_cohp_dataset))]
        pos2e_without_cohp_te_dataset = pos2e_without_cohp_dataset[int(self.split_ratio*len(pos2e_without_cohp_dataset)):]

        pos2e_without_cohp_tr_loader = DataLoader(pos2e_without_cohp_tr_dataset, batch_size=self.batch_size, shuffle=True, drop_last=True)
        pos2e_without_cohp_te_loader = DataLoader(pos2e_without_cohp_te_dataset, batch_size=self.batch_size, shuffle=True, drop_last=True)

        print("POS2EwithoutCOHP whole dataset:%s, Trainig set:%s, Test set:%s."%(len(pos2e_without_cohp_dataset),
                                                                                  len(pos2e_without_cohp_tr_dataset), len(pos2e_without_cohp_te_dataset)))
        ...
        best = 1e5
        with open("./models/POS2EwithoutCOHP_Net_%s.txt"%suffix, 'w') as file:
            file.close()
        ...
        for i in range(epochs):
            ...
            tr_loss, tr_res = cont2e_train(model, pos2e_without_cohp_tr_loader, loss_func, optimizer)
            te_loss, te_res = cont2e_evaluate(model, pos2e_without_cohp_te_loader, loss_func)
            ...
            with open("./models/POS2EwithoutCOHP_Net_%s.txt"%suffix, 'a') as file:
                file.write(training_info + "\n")   

            if learning_rate <= 1e-5 and te_res < best:
                torch.save(model, "./models/POS2EwithoutCOHP_Net_%s.pth"%suffix)
                best = te_res
