...
def restart(clean=True, criteria=None):
    ...
    if criteria == "POS2COHP":
        ...
        print(f"Key List is saved in: {filename}")
        return icohp_list_keys
    else:
        return None
...
def get_physical_encodings(ele_list):
    ...
    file_path = "./raw/physical_encodings_dict.pkl"
    if os.path.exists(file_path):
        with open(file_path, "rb") as f:
            physical_encodings_dict = pickle.load(f)
    else:
        df_radii = pd.read_excel("./raw/pre_set.xlsx",sheet_name='Radii_X')
        df_ip = pd.read_excel("./raw/pre_set.xlsx",sheet_name='IP')
