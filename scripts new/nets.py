...
class GraphMultisetTransformer(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, Conv=None, num_nodes=300,
        ...
        #self.bnp = nn.BatchNorm1d(hidden_channels)
    ...
    def forward(self, x, batch, edge_index=None):
        ...
        #return self.lin2(self.bnp(batch_x.squeeze(1)))
...
"""class CONT2E_Net(torch.nn.Module):
    def __init__(self, in_features=None, conv_dims=None, linear_dims=None, adj_conv=False, 
                 conv=SAGEConv, dropout=0., bias=True, 
                 pool=GraphMultisetTransformer, pool_dropout=0., pool_ratio=0.25, pool_heads=4, pool_seq=["GMPool_G"], pool_layer_norm=False):

        super(CONT2E_Net, self).__init__()

        self.num_conv_layers = len(conv_dims) - 1
        self.num_linear_layers = len(linear_dims) - 1
        self.adj_conv = adj_conv    
        self.dropout = dropout

        self.linear_block = torch.nn.ModuleList([Linear(in_features, linear_dims[0], bias=bias)] + 
                                                [Linear(linear_dims[i], linear_dims[i+1], bias=bias) for i in range(self.num_linear_layers)])
        self.conv_block = torch.nn.ModuleList([conv(conv_dims[i], conv_dims[i+1], bias=bias, dropout=self.dropout) for i in range(self.num_conv_layers)])
        
        if self.adj_conv:
            self.adj_block = torch.nn.ModuleList([Linear(conv_dims[i], conv_dims[i+1], bias=bias) for i in range(self.num_conv_layers)])
            
        self.pool = pool(conv_dims[-1], conv_dims[-1], 1, num_nodes=300,         
...
class CONT2E_Net(torch.nn.Module):
    def __init__(self, dim=128, N_linear=1, N_conv=3, adj_conv=False, in_features=None, bias=True,
        ...
        self.lin = Linear(in_features, dim, bias=bias)
        self.linear_block = torch.nn.ModuleList([Linear(dim, dim, bias=bias) for _ in range(self.num_linear_layers)])
        self.conv_block = torch.nn.ModuleList([conv(dim, dim, bias=bias, dropout=self.dropout) for _ in range(self.num_conv_layers)])
...
class CONT2E_add_Net(torch.nn.Module):
    def __init__(self, conv_dims=None, linear_dims=None, adj_conv=False, in_features=None, bias=True,
                 conv=SAGEConv, dropout=0., pool=global_add_pool, pool_dropout=0.):

        super(CONT2E_add_Net, self).__init__()
        ...
        #self.lin = Linear(in_features, conv_dims[0], bias=bias)
        self.linear_block = torch.nn.ModuleList([Linear(in_features, linear_dims[0], bias=bias)] + 
                                                [Linear(linear_dims[i], linear_dims[i+1], bias=bias) for i in range(self.num_linear_layers)])
        self.conv_block = torch.nn.ModuleList([conv(conv_dims[i], conv_dims[i+1], bias=bias, dropout=self.dropout) for i in range(self.num_conv_layers)])
        ...
        self.pool = pool
        self.pool_dropout = torch.nn.Dropout(pool_dropout)                
        self.ln2 = Linear(conv_dims[-1], 1, bias=bias) 
        #self.bnp = torch.nn.BatchNorm1d(conv_dims[-1])                                            
    ...
    def forward(self, data):   
        ...
        #out = self.bnp(out)
        out = self.ln2(self.pool_dropout(out))
        return out.view(-1)
