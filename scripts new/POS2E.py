...
class POS2E():
    ...
    def train_single_model_active_learning(self, suffix, dataset, setting, PRED):
        pos2e_dataset = POS2E_Dataset("./", dataset, PRED, raw_data_dict, setting, suffix).shuffle()

        initial_train_size = int(len(pos2e_dataset) * 0.5)
        pos2e_train_indices = list(range(initial_train_size))
        pos2e_pool_indices = list(range(initial_train_size, int(len(pos2e_dataset) * 0.9)))
        pos2e_val_indices = list(range(int(len(pos2e_dataset) * 0.9), len(pos2e_dataset)))


        node_feats = 41 if setting["Fake_Carbon"] else 40
        base_lr = 10 ** -3
        base_weight_decay = 10 ** -4
        epochs = 200
        loss_func = MSELoss()
        selection_ratio = 0.1
        ...
        for iteration in range(4): 
            ...
            pos2e_val_dataset = Subset(pos2e_dataset, pos2e_val_indices)

            pos2e_tr_loader = DataLoader(pos2e_tr_dataset, batch_size=96, shuffle=True, drop_last=True)
            pos2e_pool_loader = DataLoader(pos2e_pool_dataset, batch_size=96, shuffle=False)
            pos2e_val_loader = DataLoader(pos2e_val_dataset, batch_size=96, shuffle=False)
            ...
            if iteration == 0:
                ...
                model = CONT2E_Net(dim=256, N_linear=1, N_conv=3, adj_conv=False, in_features=node_feats, bias=False,
                                  conv=pyg_GCNLayer, dropout=0., pool=GraphMultisetTransformer,
                                  pool_ratio=0.25, pool_heads=4, pool_seq=["GMPool_G"], pool_layer_norm=False)
                lr = base_lr
            ...
            weight_decay = base_weight_decay * (1 ** iteration)
            ...
            for epoch in range(epochs):
                tr_loss, tr_res = cont2e_train(model, pos2e_tr_loader, loss_func, optimizer)
                te_loss, te_res = cont2e_evaluate(model, pos2e_pool_loader, loss_func)
                vl_loss, vl_res = cont2e_evaluate(model, pos2e_val_loader, loss_func)
            ...
            pool_losses = cont2e_cal(model, pos2e_pool_loader)
