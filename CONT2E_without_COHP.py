...
class CONT2E_without_COHP:
    ...
    def train_single_model(self, setting,icohp_list_keys):
        ...
        #####################################################################################################################
        cont2e_dataset = CONT2EwithoutCOHP_Dataset("./", Element_List=self.Element_List, setting=setting,icohp_list_keys=icohp_list_keys)#.shuffle() shuffle bas been done in main()

        cont2e_train_dataset = cont2e_dataset[:int(self.split_ratio[0]*len(cont2e_dataset))]
        cont2e_valid_dataset = cont2e_dataset[int(self.split_ratio[0]*len(cont2e_dataset)):int((self.split_ratio[0]+self.split_ratio[1])*len(cont2e_dataset))]
        cont2e_test_dataset = cont2e_dataset[int((self.split_ratio[0]+self.split_ratio[1])*len(cont2e_dataset)):]

        cont2e_train_loader = DataLoader(cont2e_train_dataset, batch_size=self.batch_size, shuffle=True, drop_last=True)
        cont2e_valid_loader = DataLoader(cont2e_valid_dataset, batch_size=self.batch_size, shuffle=False, drop_last=False)
        cont2e_test_loader = DataLoader(cont2e_test_dataset, batch_size=self.batch_size, shuffle=False, drop_last=False)

        print("CONT2EwithoutCOHP whole dataset:%s, Trainig set:%s, Valid set:%s, Test set:%s."%(len(cont2e_dataset), len(cont2e_train_dataset), len(cont2e_valid_dataset), len(cont2e_test_dataset)))
        ...
        best = 1e5
        log_file = "./models/CONT2EwithoutCOHP_Net_%s_%s.txt"%(suffix,file_name_current_time)
        ...
        for i in range(epochs):
            ...
            train_loss, train_res = cont2e_train(model, cont2e_train_loader, loss_func, optimizer)
            valid_loss, valid_res = cont2e_evaluate(model, cont2e_valid_loader, loss_func)
            test_loss, test_res = cont2e_evaluate(model, cont2e_test_loader, loss_func)
            ...
            if learning_rate <= 1e-5 and valid_res < best:
                torch.save(model, "./models/CONT2EwithoutCOHP_Net_%s%s.pth"%(suffix,file_name_current_time))
                best = valid_res
